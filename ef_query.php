...
class Ef_queries
{
    ...
    public function ef_register_input_settings_custom_table()
    {
        ...
        if ($query_select < 1 && $data['id_input'] != '') {
            $insert = "INSERT INTO {$wpdb->prefix}ef_input_settings (id_input, nom, input_type, input_valeur, css_class, input_id_form) VALUES ('{$data['id_input']}', '{$data['nom']}', '{$data['input_type']}', '{$data['input_valeur']}', '{$data['input_css']}', '{$data['input_id_form']}')";
            $wpdb->query($insert);
